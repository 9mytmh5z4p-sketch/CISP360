...
## How can you let users choose<br> what your program does?
...
Here's a complete menu program that runs once and performs a calculation. This demonstrates the complete three-part structure: display menu, get choice, process choice.
...
Building on the simple menu, we can use menu choices to control how a for-each loop processes vector data. This gives users choice over operations on collections. The menu choice determines which operation the for-each loop performs on each element. Notice the loop structure is identical - only the operation inside changes.
