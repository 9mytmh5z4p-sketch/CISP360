...
## This has to<br> do with Scope

Recall, **scope** is the area where a variable can be accessed. 

It's the area where the variable is alive.

## Main() is one<br> level of scope

```cpp
int main() {

    // any variable we create here is alive.
...
int main() {
    ...
    // One level of scope for main

    for (int i : scores) {

        // we created another level of scope
...
Every for-each loop we create will create a level of scope.

But, these will all be at the same level if they are all in main().
...
## Fosters Encapsulation<br> and Information Hiding
...
Scope tells you where to look for variable definitions. If you see a variable used in a function, you know it's either declared in that function, passed as a parameter, or is a global—you don't have to search the entire codebase.
