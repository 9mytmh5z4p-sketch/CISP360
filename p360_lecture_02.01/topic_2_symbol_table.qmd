...
+ **Missing entry** — the compiler searches the symbol table and cannot find `count`.
...
+ **Duplicate entry** — the compiler already has an `x` in this scope.
...
+ **Wrong type** — the table says `age` is an `int`, but the assignment provides text.
